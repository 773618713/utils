    ...
    define([], function() {
        return [
            ...
            function($scope,$httpService,config,params,$routeParams,eventBusService,controllerName,loggingService) {
	            ...
	            //接收选中的按钮
            	eventBusService.subscribe(controllerName, 'appPart.load.content.checkButton', function(event, m2) {
            		$scope.buttonName = m2.buttonName;
            		//$scope.currentButtonCode = m2.BUTTON_CODE;
            		//if (m2.BUTTON_CODE != null)
            		for (var i = 0; i < $scope.buttonData.length; i++) {
            			if ($scope.buttonData[i].BUTTON_CODE == m2.BUTTON_CODE) {
            				$scope.buttonData[i].BUTTON_STYLE = 'primary-current';
						}else {
							$scope.buttonData[i].BUTTON_STYLE=$scope.buttonData[i].BUTTON_STYLE.replace("-current","");
						}
					}
